...
app.post('/searchByMark', function(request, response) 
{
    ...
    if (mark!="not defined" && mark!="body undefined" && !isNaN(mark[1]))
	{
		//aceptable input
		//search for a list of student with that mark
		...
		var studentMark = studentManager.searchByMark(type,mark[1]);
		//if exists
		if (studentMark != null)
		{
			response.writeHead(200, headers);
			response.end(JSON.stringify(studentMark));
		}
		else
		{
			response.writeHead(404, headers);
